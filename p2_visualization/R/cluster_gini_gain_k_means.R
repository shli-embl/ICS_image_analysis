...
#opt<-data.frame(input="/scratch/shli/icstag/data/downsampling.txt",local = "/g/steinmetz/shli/gitlab/ics_tagging/inputs/localization.txt", fset="/g/steinmetz/shli/gitlab/ics_tagging/inputs/feature_set_GFP_relevant.txt",batch="set2_run1",output="/g/steinmetz/shli/gitlab/ics_tagging/test.out")
### load input table
...
### MDS
#### normalize to 0-1
dist_mat_sym <- (dist_mat_sym-0.5)*2
mds_tag<-cmdscale(as.dist(dist_mat_sym), eig=TRUE, k=2)

#pdf("/g/steinmetz/shli/gitlab/ics_tagging/plots/49_tag_MDS.pdf",width = 10,height = 5)
#ggplot() + geom_text(aes(x = mds_tag$points[,1],y = mds_tag$points[,2],label = rownames(mds_tag$points))) +
#  labs(x = "MDS layout 1", y = "MDS layout 2") +
#  theme_bw() + theme(panel.grid.major = element_blank(), panel.border = element_rect(colour = "black", fill=NA),
#                     panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
#dev.off()
cl_list<-list()
#for(k in c(5:(length(table(indata$V3))-2)))
for(k in c(2:40))
{
  cl_list[[paste0("k_",k)]]<-kmeans(mds_tag$points,k)$cluster
}
...
write.table(anno,paste0(opt$output,".local.txt"),row.names = T,col.names = T,sep="\t",quote=F)
